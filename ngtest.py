...
# Without push:
#   nghttpd -w30 -W30 --no-tls --htdocs=/vfs0/nghttp_data/ --no-tc-cache v 8080


def call_nghttp_with_push(fo, fl, n):
	start_time = time.time()
	for x in xrange(n):
		check_call(["nghttp", "-w30", "-W30", "http://localhost:8080/"], stdout = fo, stderr = fl)
...
def call_nghttp_without_push(fo, fl, n):
	start_time = time.time()
	for x in xrange(n):
		check_call(["nghttp", "-a", "-w30", "-W30", "http://localhost:8080/"], stdout = fo, stderr = fl)
	time_taken = round(((time.time() - start_time) * 1000) / n, 2)
        print("Avg time taken for client execution: %s ms" % time_taken)
	s = mmap.mmap(fo.fileno(), 0)
	if s.find('Not Found') != -1:
